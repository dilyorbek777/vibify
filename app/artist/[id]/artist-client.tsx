      ...
      <div className="relative bg-gradient-to-b from-primary/20 via-background/60 to-background overflow-hidden border-b border-border/10">
        <div className="max-w-7xl mx-auto px-6 pt-16 pb-10 flex flex-col md:flex-row items-center md:items-end gap-8 relative z-10">
          ...
          <div className="flex-1 text-center md:text-left space-y-3 min-w-0">
            {artist.verified && (
              <Badge variant="secondary" className="bg-primary/10 text-primary border-transparent rounded-full px-3 py-1 font-semibold gap-1 text-xs tracking-wide uppercase pointer-events-none">
                <Check className="h-3.5 w-3.5 stroke-[3]" /> Verified Artist
              </Badge>
            )}
            
            <h1 className="text-4xl md:text-7xl font-black tracking-tight leading-none text-foreground drop-shadow-sm py-1">
              {artist.name}
            </h1>
      ...
      <main className="max-w-7xl mx-auto px-6 mt-4">
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as any)} className="w-full">
          <TabsList className="bg-muted/40 p-1 border border-border/20 rounded-xl mb-8">
            <TabsTrigger value="songs" className="rounded-lg px-4 py-2 font-semibold text-sm">Popular Tracks</TabsTrigger>
            <TabsTrigger value="albums" className="rounded-lg px-4 py-2 font-semibold text-sm">Discography</TabsTrigger>
            <TabsTrigger value="about" className="rounded-lg px-4 py-2 font-semibold text-sm">About</TabsTrigger>
          </TabsList>

          {/* Songs Content Grid View */}
          <TabsContent value="songs" className="space-y-4 focus-visible:outline-none">
            <div className="flex items-center justify-between border-b border-border/10 pb-2 mb-2 text-xs font-semibold tracking-wider text-muted-foreground uppercase px-4">
              ...
              {albums.length > 0 ? albums.map(album => (
                ...
                >
                  ...
                  <div className="space-y-1 px-1">
                    <CardTitle className="text-sm font-bold tracking-tight truncate group-hover:text-primary transition-colors">{album.name}</CardTitle>
            ...
            <Card className="border border-border/20 bg-card/20 overflow-hidden rounded-2xl shadow-inner">
              <CardContent className="p-6 md:p-8 space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center gap-6 pb-6 border-b border-border/10">
                  ...
                  <div>
                    <h3 className="text-xl font-bold tracking-tight mb-0.5">Artist Information</h3>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pt-4 border-t border-border/10">
                  <div className="bg-muted/30 p-4 rounded-xl border border-border/10">
                    <p className="text-xs font-semibold mb-1 text-muted-foreground uppercase tracking-wider">Albums</p>
                    <p className="text-xl font-black text-foreground tabular-nums">{albums.length}</p>
                  </div>
                  <div className="bg-muted/30 p-4 rounded-xl border border-border/10">
                    <p className="text-xs font-semibold mb-1 text-muted-foreground uppercase tracking-wider">Popular Tracks</p>
                    <p className="text-xl font-black text-foreground tabular-nums">{popularSongs.length}</p>
                  </div>
                  <div className="bg-muted/30 p-4 rounded-xl border border-border/10">
                    <p className="text-xs font-semibold mb-1 text-muted-foreground uppercase tracking-wider">Genres</p>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {artist.genres.slice(0, 2).map(g => (
                        <Badge key={g} variant="outline" className="text-[10px] py-0 border-border/60 font-bold bg-background/50 text-muted-foreground pointer-events-none">{g}</Badge>
                      ))}
                    </div>
                  </div>
                  <div className="bg-muted/30 p-4 rounded-xl border border-border/10">
                    <p className="text-xs font-semibold mb-1 text-muted-foreground uppercase tracking-wider">Status</p>
